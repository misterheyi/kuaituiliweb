...
<head>
    ...
    <title>${info.title}_推理名人_推理作家_酷爱推理</title>
...
<body>
	...
	<div class="mainContent">
		<div class="main clearfix">
			<p>您当前位置：<a href="${ctx}index.html">首页</a> > <a href="${ctx}info.html">短文</a> > <span>${info.title}</span></p>
			...
			<div class="read_link  ">
			 	<h1>相关文章</h1>
				 <ul class="short_list">
				 	<c:forEach items="${infoRandomList}" var="randomInfo">
				 		<li>
							<a class="img_a" href="${ctx}info/detail/${randomInfo.id}.html"  title="${randomInfo.title}"><img src="${imgContext}${randomInfo.logoUrl}" alt="" title="${randomInfo.title}" />
								<h3>${randomInfo.title}</h3>
								<p class="infotxt">${randomInfo.description}</p>
								<p class="readmore"><fmt:formatDate value="${randomInfo.createTime}" pattern="yyyy/MM/dd"/>&nbsp;&nbsp;&nbsp;&nbsp;<span>阅读(${randomInfo.clickTimes})</span></p>
							</a>
						</li>
				 	</c:forEach>
				 </ul>
